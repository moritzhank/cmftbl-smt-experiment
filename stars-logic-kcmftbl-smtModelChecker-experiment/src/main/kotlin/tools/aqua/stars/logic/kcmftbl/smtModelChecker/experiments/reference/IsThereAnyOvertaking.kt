...
fun main() {
  val res01 = overtakingEventsInCity("01", arrayOf(2, 7, 10, 15, 18, 19, 22, 25, 26, 29, 32, 33, 36, 42, 47, 52, 53, 55, 58, 59, 61, 62, 64, 67, 69, 73, 78, 86, 88, 94))
  res01.forEach {
    println("Town 01; Seed ${it.seed}; Segment ${it.segment}; Vehicle ${it.vehicleId}")
  }
  println()
  val res02 = overtakingEventsInCity("02", arrayOf(0, 4, 5, 8, 12, 13, 16, 17, 21, 23, 24, 27, 30, 31, 34, 35, 38, 39, 41, 43, 45, 46, 48, 49, 50, 54, 57, 60, 65, 66, 68, 70, 76, 77, 79, 80, 81, 82, 83, 84, 93, 96, 97, 99, 100))
  res02.forEach {
    println("Town 02; Seed ${it.seed}; Segment ${it.segment}; Vehicle ${it.vehicleId}")
  }
  println()
  val res10HD = overtakingEventsInCity("10HD", arrayOf(1, 3, 6, 9, 11, 14, 20, 28, 37, 40, 44, 51, 56, 72, 74, 75, 85, 87, 89, 90, 91, 92, 95, 98))
  res10HD.forEach {
